...
写入仍采用同步写穿语义：数据只存在于后台缓存时，GugleFS 不会提前返回成功。SFTP 会在每次写入关闭远端句柄后回读并逐字节校验已确认范围，截断后也会确认远端长度；WinFsp 的受限写入不会越过当前文件末尾，分配大小请求不会改变可见文件长度。FTP/FTPS 会先上传同目录临时文件、校验远端长度，再重命名替换目标；上传或校验失败会保留原文件。任一协议写入失败后，该文件句柄的后续写入和刷新都会继续报告错误，VFS 释放也会保留失败状态；WinFsp 的 `close` 回调本身无法单独返回错误。SFTP/WebDAV 的完整临时替换、有界加密溢写和崩溃恢复日志方案记录在 [WRITE_STRATEGY.zh_CN.md](docs/WRITE_STRATEGY.zh_CN.md)，其中也列出了启用写回前必须完成的原生平台和协议测试。
