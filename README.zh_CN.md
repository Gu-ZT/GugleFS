...
写入仍采用同步写穿语义：数据只存在于后台缓存时，GugleFS 不会提前返回成功。已经评审的分块、临时对象替换、有界加密溢写和崩溃恢复日志方案记录在 [WRITE_STRATEGY.zh_CN.md](docs/WRITE_STRATEGY.zh_CN.md)，其中也列出了启用写回前必须完成的原生平台和协议测试。
