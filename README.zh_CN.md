...
- **保留远端信息** —— 将协议可获取的时间戳和存储容量投影为原生文件系统元数据，并短暂缓存容量查询
- **保持更新** —— 支持可关闭的启动自动检查和手动检查，并直接前往最新 GitHub Release
...
界面支持简体中文和英文。首次启动时 GugleFS 会跟随系统语言，认证页和工作区均可随时切换语言，原生托盘菜单会立即同步；用户选择会保存在本机并用于后续启动。

### 远程访问性能

共享 VFS 层对 FTP、SFTP 和 WebDAV 启用了有界短期缓存：元数据缓存 3 秒、目录缓存 2 秒、未找到结果缓存 1 秒，最多保留 4096 项。同一路径的并发冷目录读取会合并为一个远端请求，挂载完成后会异步预取根目录；WinFsp 在一轮分页枚举中复用稳定的已排序快照，FTP 则使用 `CWD` 校验映射根目录，避免为了根元数据读取完整列表。每个打开文件还会进行 1 MiB 顺序预读；创建、写入、截断、重命名和删除会更新或失效相关缓存，跨句柄写入也会使旧预读数据失效。
...
写入仍采用同步写穿语义：数据只存在于后台缓存时，GugleFS 不会提前返回成功。SFTP 会在每次写入关闭远端句柄后回读并逐字节校验已确认范围，截断后也会确认远端长度；WinFsp 的受限写入不会越过当前文件末尾，分配大小请求不会改变可见文件长度。FTP/FTPS 会先上传同目录非隐藏临时文件、校验远端长度，再重命名替换目标，以兼容禁止点号开头文件名的服务器；上传或校验失败会保留原文件。任一协议写入失败后，该文件句柄的后续写入和刷新都会继续报告错误，VFS 释放也会保留失败状态；WinFsp 的 `close` 回调本身无法单独返回错误。SFTP/WebDAV 的完整临时替换、有界加密溢写和崩溃恢复日志方案记录在 [WRITE_STRATEGY.zh_CN.md](docs/WRITE_STRATEGY.zh_CN.md)，其中也列出了启用写回前必须完成的原生平台和协议测试。
...
更新检查默认开启，也可在侧边栏关闭。GugleFS 会比较当前安装包的语义化版本与最新 GitHub Release；GitHub API 不可用时，通过代理读取仓库版本作为回退，发现新版本后显示下载入口。应用不会静默下载或安装更新，因此更新流程不会绕过正常退出与卸载路径。
