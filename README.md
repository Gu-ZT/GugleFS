...
The shared VFS uses bounded short-lived caches: metadata and completed directory listings for 30 seconds, negative lookups for 3 seconds, and at most 4096 entries. SFTP directory enumeration keeps a server cursor open on a dedicated channel and fetches at most 256 entries per page with linear accumulation. Waiting for that directory channel does not hold the main SFTP connection lock, so metadata and file operations remain concurrent with enumeration. WinFsp and FUSE return pages incrementally, preserve a stable per-handle order, and release unfinished cursors when an enumeration is closed; WinFsp also uses a constant-time marker index. Explorer probes for unknown names such as `desktop.ini` are answered from the active directory name view once enumeration starts, avoiding both complete parent-directory scans and serial per-name SFTP metadata requests. A WinFsp name index only answers negative lookups after the directory has been fully enumerated; while a page stream is incomplete, unknown names fall back to remote metadata so existing files outside the first page remain openable. Repeated enumeration restarts on one WinFsp handle replay the stable snapshot and continue its existing remote cursor while the directory is unchanged; successful local create, remove, or rename operations refresh it. WinFsp volume-information callbacks return cached or compatibility capacity immediately and refresh SFTP `statvfs` or WebDAV quota data asynchronously; FUSE mounts retain cached remote capacity reporting. Root prefetch is intentionally disabled so it cannot occupy the serialized SFTP connection ahead of the foreground browser request. Servers that reject a second SFTP channel, plus FTP and WebDAV, retain their compatible full-list fallback. FTP validates mapping roots with `CWD` instead of reading a full listing for root metadata. Open files use 1 MiB sequential read-ahead. Create, write, truncate, rename, and delete operations update or invalidate relevant cache entries, including read-ahead buffers owned by other handles.
...
Writes remain synchronous and write-through: GugleFS does not report success while data exists only in a background cache. SFTP uses a standard write-and-truncate open operation when replacing an existing same-name file, reads back and compares every acknowledged range after closing its remote write handle, and verifies the remote length after truncation. WinFsp constrained writes cannot pass the current EOF, and allocation-size requests do not change the visible file length. FTP/FTPS uploads a non-hidden same-directory temporary file for compatibility with servers that reject leading-dot names, verifies its remote length, and renames it over the target, so upload or verification failures preserve the original file. After any protocol write fails, later writes and flushes on that file handle continue reporting an error, and VFS release preserves the failure state; WinFsp's `close` callback cannot report an error by itself. The complete staged-replacement, bounded encrypted spill, and crash-journal design for SFTP/WebDAV is documented in [WRITE_STRATEGY.md](docs/WRITE_STRATEGY.md), together with the native-platform and protocol tests required before write-back can be enabled.
...
The current source release line is `0.16.2`; the matching user-visible changes are tracked in [CHANGES.md](CHANGES.md) and [CHANGES.zh_CN.md](CHANGES.zh_CN.md).
