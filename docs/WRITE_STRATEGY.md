...
The current VFS is write-through. It serializes writes for each open handle and waits for `RemoteFileSystem::write` before returning success to WinFsp or FUSE. SFTP writes the requested range directly. FTP and WebDAV use read-modify-write where the protocol does not expose a portable random-write operation. WebDAV applies an ETag or Last-Modified precondition when the server provides one. `flush` propagates backend errors, but there is no durable local write-back journal.
