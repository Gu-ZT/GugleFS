...
- [ ] 建立可注入的协议测试服务和集成测试（SFTP 已通过；Pure-FTPd 已稳定前台运行并按 chroot 根修正测试路径，等待同轮 CI 复验）
...
- [ ] 使用本地 FTP / SFTP 容器执行协议集成测试（SFTP 已通过；Pure-FTPd 已稳定前台运行并按 chroot 根修正测试路径，等待同轮 CI 复验）
