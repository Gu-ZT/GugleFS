...
- [ ] 建立可注入的协议测试服务和集成测试（SFTP 已通过；Pure-FTPd 已稳定运行，已修复真实测试发现的 MLST/MLSD 扩展兼容问题，等待同轮 CI 复验）
...
- [ ] 使用本地 FTP / SFTP 容器执行协议集成测试（SFTP 已通过；Pure-FTPd 已稳定运行，已修复真实测试发现的 MLST/MLSD 扩展兼容问题，等待同轮 CI 复验）
