...
- [x] 将 WinFsp 分页目录文件打开修复整理为 0.16.2 版本
...
- [x] 增加每映射并发限流、分级超时和一次退避重试；FTP 丢弃失效会话，SFTP 重建会话，WebDAV 复用 HTTP 连接池重连
...
- [x] FTP：写入使用兼容禁止隐藏文件服务器的同目录临时文件、远端长度校验和重命名替换，上传失败时保留原目标
- [x] SFTP：支持密码、本地私钥和粘贴私钥认证
- [x] SFTP：同步回读校验写入范围并确认截断后的远端长度，失败状态传播到后续写入和刷新
- [x] SFTP：同名覆盖清空文件时兼容拒绝路径级 `SETSTAT` 截断的服务器
...
- [x] FTP 解析 MLST/MLSD 修改时间，SFTP 读取访问/修改时间和 `statvfs`，WebDAV 读取创建/修改时间与配额
...
- [x] Windows：目录分页未完成时对未见名称回退远端元数据，避免打开首分页之外的文件失败
...
- [ ] 处理休眠、网络切换和系统关机
- [x] 挂载/卸载运行于 Tauri 异步任务，并向前端推送 `mounting`、`unmounting` 和最终状态事件
...
- [x] 增加复用当前凭据、代理、SSH 主机验证和 MFA 的远程路径选择
...
- [x] 增加开机启动选项
- [x] 增加脱敏结构化日志、1 MiB 日志轮转和 JSON 诊断报告导出
- [x] 增加启动自动检查、手动检查和 GitHub Release 下载入口（不在应用内静默安装）
- [x] 使用运行标记检测崩溃、强退或卸载失败，并在下次解锁后恢复已保存凭据的挂载状态
- [x] 完善键盘操作、对话框初始焦点、屏幕阅读器上下文和实时状态/错误播报
- [x] 增加英文界面与多语言切换，同步原生托盘菜单，并按系统语言初始化和持久化用户偏好
...
- [ ] 在真实 WinFsp 挂载上验证同名 Unicode JAR 覆盖上传及受限写入边界
