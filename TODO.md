...
- [ ] 定义完整 VFS 接口：`lookup`、`getattr`、`readdir`、`open`、`read`、`write`、`flush`、`release`
- [ ] 补充目录创建、重命名、删除、截断和时间戳操作
- [ ] 建立统一错误码，将远端错误映射为 POSIX / Windows 文件系统错误
- [ ] 定义文件句柄、目录句柄与并发访问生命周期
- [ ] 设计元数据缓存、目录缓存和负缓存策略
- [ ] 设计分块读写、预读、写回与失败恢复策略
- [ ] 增加限流、超时、重试和断线重连
- [ ] 为核心语义编写与平台无关的测试套件
